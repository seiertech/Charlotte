...
def run_rewrite_chapter(
    ...
    while revision <= max_revisions:
        ...
        rewrite_task = (
            ...
            f"Apply all editorial directives. Remove production artifacts. "
            f"Use exact Model Bible terminology. "
            f"Cut repetitive prose and add new substantive content (mechanism explanations, "
            f"failure modes, hard cases, counterexamples).\n\n"
            f"CRITICAL — CHARACTER REPLACEMENT (this is the most common failure):\n"
            f"The ONLY named characters permitted are: Maren, Callum, Grace, Marcus, Elena.\n"
            f"You MUST replace EVERY other name with one of these five, using this mapping:\n"
            f"  - Priya, Aisha, Nina, Lena → Elena (or Grace)\n"
            f"  - Sam, Alex, Tom, Eli → Callum (or Marcus)\n"
            f"  - Jordan, Dev, Javier, Rafael → Marcus (or Grace)\n"
            f"  - Jamie, Mira, Ade → Maren\n"
            f"Match by layer where possible: Body→Maren, Wiring→Callum, Habit→Grace, "
            f"Room→Marcus, Story→Elena. "
            f"Scan the ENTIRE chapter and replace ALL occurrences — including in dialogue, "
            f"possessives (Priya's → Elena's), and passing mentions. "
            f"Do NOT leave a single non-Bible name anywhere. Double-check before finishing.\n\n"
